...
var ICalCalendar = function(_data) {
	...
	generate = function(calendar) {
        ...
        // TTL
        if(data.ttl) {
            g += 'X-PUBLISHED-TTL:PT' + data.ttl + '\n';
            g += 'REFRESH-INTERVAL;VALUE=DURATION:P' + data.ttl + '\n';
        }
 ...
 * }
    ...
    this.ttl = function(ttl) {
        ...
        // TTL / weeks
        if(Math.floor(ttl / 604800)) {
            duration += Math.floor(ttl / 604800) + 'W';
            ttl %= 604800;
        }

        // TTL / days
        if(Math.floor(ttl / 86400)) {
            duration += Math.floor(ttl / 86400) + 'D';
            ttl %= 86400;
        }

        // TTL / hours
        if(Math.floor(ttl / 3600)) {
            duration += Math.floor(ttl / 3600) + 'H';
            ttl %= 3600;
        }

        // TTL / minutes
        if(Math.floor(ttl / 60)) {
            duration += Math.floor(ttl / 60) + 'M';
            ttl %= 60;
        }

        if(ttl > 0) {
            duration += Math.floor(ttl) + 'S';
        }

        this.data.ttl = duration;
        return this;
    };
	...
	this.clear();
	for(i in _data) {
		if(_data.hasOwnProperty(i) && ['domain', 'prodId', 'name', 'timezone', 'events'].indexOf(i) > -1) {
			this[i](_data[i]);
		}
	}
};

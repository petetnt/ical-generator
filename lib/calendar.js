...
var ICalCalendar = function(_data) {
	var data = {},
		attributes = ['domain', 'prodId', 'name', 'timezone', 'method', 'xMSOlkForceInspectorOpen', 'events'],
		generate,
		i;
	...
	generate = function(calendar) {
        ...
        if(data.xMSOlkForceInspectorOpen) {
            g += 'X-MS-OLK-FORCEINSPECTOROPEN:' + data.xMSOlkForceInspectorOpen + '\r\n';
        }
 ...
 * }
    ...
    /**
	 * Set/Get your feed's X-MS-OLK-FORCEINSPECTOROPEN.
	 * Used to set `X-MS-OLK-FORCEINSPECTOROPEN`.
	 *
	 * @param [method] Method
	 * @since 0.2.1
	 * @returns {ICalCalendar}
	 */
  	this.xMSOlkForceInspectorOpen = function(xMSOlkForceInspectorOpen) {
		if(!xMSOlkForceInspectorOpen) {
			return data.xMSOlkForceInspectorOpen;
		}

		data.xMSOlkForceInspectorOpen = xMSOlkForceInspectorOpen.toString();
		return this;
	};

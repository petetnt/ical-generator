...
var ICalCalendar = function(_data) {
	...
	generate = function(calendar) {
        ...
        // TTL
        if(data.ttlStr) {
            g += 'X-PUBLISHED-TTL:PT' + data.ttlStr + '\n';
            g += 'REFRESH-INTERVAL;VALUE=DURATION:P' + data.ttlStr + '\n';
        }
 ...
 * }
    ...
    this.ttl = function(ttl) {
        ...
        // TTL / weeks
        if(Math.floor(t / 604800)) {
            duration += Math.floor(t / 604800) + 'W';
            t %= 604800;
        }
        ...
        // TTL / hours
        if(Math.floor(t / 3600)) {
            duration += Math.floor(t / 3600) + 'H';
            t %= 3600;
        }

        // TTL / minutes
        if(Math.floor(t / 60)) {
            duration += Math.floor(t / 60) + 'M';
            t %= 60;
        }
	...
	this.clear();
	for(i in _data) {
		if(_data.hasOwnProperty(i) && ['domain', 'prodId', 'name', 'timezone', 'ttl', 'events'].indexOf(i) > -1) {
			this[i](_data[i]);
		}
	}
};

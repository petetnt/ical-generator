...
var ICalEvent = function(_data) {
    var attributes = ['id', 'uid', 'start', 'end', 'stamp', 'timestamp', 'allDay', 'floating', 'repeating', 'summary', 'location', 'description', 'xAltDesc', 'organizer', 'attendees', 'alarms', 'method', 'status', 'url', 'sequence', 'class', 'xMicrosoftCDOImportance', 'transp'
                     ],
        vars,
        i,
        data;
    ...
    /**
	 * Set/Get the event's x-alt-desc
	 *
	 * @param {String} xAltDesc
	 * @since 0.2.0
	 * @returns {ICalEvent|String}
	 */
    this.xAltDesc = function(xAltDesc) {
        if(!xAltDesc) {
            return data.description;
        }

        data.xAltDesc = xAltDesc.toString();
        return this;
    };
    ...
    /**
	 * Set/Get the event's X-MICROSOFT-CDO-IMPORTANCE
	 *
	 * @param {String} value X-MICROSOFT-CDO-IMPORTANCE value
	 * @since 0.2.1
	 * @returns {ICalEvent|String}
	 */

    this.xMicrosoftCDOImportance = function(xMicrosoftCDOImportance) {
        if(!xMicrosoftCDOImportance) {
            return data.xMicrosoftCDOImportance;   
        }

        data.xMicrosoftCDOImportance = xMicrosoftCDOImportance.toString();
        return this;
    }    

    /**
	 * Set/Get the event's transp-value
	 *
	 * @param {String} transp Transparency
	 * @since 0.2.1
	 * @returns {ICalEvent|String}
	 */

    this.transp = function(transp) {
        if(!transp) {
            return data.transp;   
        }

        data.transp = transp.toString();
        return this;
    }    


    /**
	 * Set/Get the event's sequence
	 *
	 * @param {String} url Class
	 * @since 0.2.1
	 * @returns {ICalEvent|String}
	 */
    ...
    this.generate = function(calendar) {
        ...
        // SUMMARY
        g += 'SUMMARY;LANGUAGE=en:' + tools.escape(data.summary) + '\r\n';
        ...
        // X-ALT-DESC
        if(data.xAltDesc) {
            g += 'X-ALT-DESC:' +  tools.escape(data.xAltDesc) + '\r\n';
        }
        ...
        if(data.xMicrosoftCDOImportance) {
            g += 'X-MICROSOFT-CDO-IMPORTANCE:' + data.xMicrosoftCDOImportance + '\r\n'; 
        }

        if(data.xMicrosoftCDOImportance) {
            g += 'TRANSP:' + data.transp + '\r\n'; 
        }
